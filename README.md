...
1. Create a folder named `dev-reset-toolkit` and place these files inside it:
   - `reset.php` (main plugin bootstrap with header)
   - `dev-reset-toolkit.php` (compatibility loader)
   - `includes/`
   - `assets/`
   - `README.md` (optional)
...
### Installing from WP Pusher / GitHub repo

- WP Pusher commonly validates `<repo-name>.php` in the repository root.
- This repository includes `reset.php` in the root so a repo named `reset` has a valid header entrypoint.
- In WP Pusher, keep **Repository subdirectory** empty.
